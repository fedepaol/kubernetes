...
const sctpPrefix = "SCTP_SERVE_PORT_"
...
The server also supports serving the requested data on SCTP ports. To enable serving on SCTP ports, the environment variable must start with SCTP_. For example, an environment like "SCTP_SERVE_PORT_9001 will make the subcommand to serve via SCTP on port 9001"`,
...
func main(cmd *cobra.Command, args []string) {
	for _, vk := range os.Environ() {
		...
		if strings.HasPrefix(key, sctpPrefix) {
